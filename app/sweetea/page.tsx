...
// --- DATA CHI TIẾT SWEETEA ---
const sweeteaData = {
  name: "Sweetea",
  tagline: "Thanh xuân ngọt ngào",
  price: "13.800.000 VNĐ",
  signatureHex: "#EC4899", // Màu hồng rực rỡ làm điểm nhấn
  description: "Ngọt ngào như một ly trà sữa, Powelldd Sweetea mang đến thiết kế nhỏ gọn, kute và cực kỳ nữ tính. Lớp sơn pastel thời thượng kết hợp cùng các đường nét bo tròn mềm mại biến Sweetea thành món phụ kiện không thể thiếu, giúp bạn tự tin tỏa sáng rạng rỡ mỗi ngày khi đến trường hay dạo phố.",
  colors: [
    { name: "Kem trà sữa", hex: "#F3E5AB", img: "/images/powelldd/sweetea/kem-tra-sua.png" },
    { name: "Xanh Matcha", hex: "#D9F99D", img: "/images/powelldd/sweetea/xanh-matcha.png" },
    { name: "Xám sương mai", hex: "#D1D5DB", img: "/images/powelldd/sweetea/xam-suong-mai.png" },
    { name: "Vàng ánh dương", hex: "#FDE047", img: "/images/powelldd/sweetea/vang-anh-duong.png" }
  ],
  specs: [
    { icon: <Zap size={20} strokeWidth={1.5}/>, label: "Động cơ", value: "600W - Bền bỉ, êm ái" },
    ...
    { icon: <Gauge size={20} strokeWidth={1.5}/>, label: "Vận tốc tối đa", value: "35 km/h" },
...
// --- DANH SÁCH ẢNH TÍNH NĂNG SWEETEA (ĐÃ CẬP NHẬT ĐỦ 7 ẢNH) ---
const featureImages = [
  "/images/vehicles/sweetea/anh-1.jpg",
  "/images/vehicles/sweetea/anh-2.jpg",
  "/images/vehicles/sweetea/anh-3.jpg",
  "/images/vehicles/sweetea/anh-4.jpg",
  "/images/vehicles/sweetea/anh-5.jpg",
  "/images/vehicles/sweetea/anh-6.jpg",
  "/images/vehicles/sweetea/anh-7.jpg"
];

export default function SweeteaDetail() {
  const [activeColor, setActiveColor] = useState(sweeteaData.colors[0]);

  // HÀM XỬ LÝ VUỐT (SLIDE) ĐỔI MÀU
  const handleDragEnd = (event: any, info: any) => {
    const swipeThreshold = 50; // Khoảng cách vuốt tối thiểu
    const currentIndex = sweeteaData.colors.findIndex(c => c.name === activeColor.name);

    if (info.offset.x < -swipeThreshold) {
      // Vuốt sang trái -> Sang ảnh tiếp theo
      const nextIndex = (currentIndex + 1) % sweeteaData.colors.length;
      setActiveColor(sweeteaData.colors[nextIndex]);
    } else if (info.offset.x > swipeThreshold) {
      // Vuốt sang phải -> Về ảnh trước đó
      const prevIndex = (currentIndex - 1 + sweeteaData.colors.length) % sweeteaData.colors.length;
      setActiveColor(sweeteaData.colors[prevIndex]);
    }
  };

  return (
    <main className={`min-h-screen bg-white text-pink-950 ${elegantFont.className} font-light`}>
      
      {/* HEADER HỒNG PASTEL TỐI GIẢN */}
      <header className="fixed w-full top-0 bg-white/80 backdrop-blur-xl z-50 border-b border-pink-100">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          <Link href="/" className="flex items-center gap-2 text-pink-400 hover:text-pink-600 transition-colors group">
            <ArrowLeft size={20} className="group-hover:-translate-x-1 transition-transform" />
            <span className="text-sm font-semibold tracking-widest uppercase mt-0.5">Trở về</span>
          </Link>
          <div className="flex flex-col items-end">
            <h1 className="text-xl font-semibold tracking-widest uppercase text-pink-600 leading-none">Minh Anh</h1>
            <span className="text-[9px] font-medium tracking-widest text-pink-400 uppercase mt-1">E-Scooter</span>
          </div>
        </div>
      </header>
      ...
      <section className="pt-28 pb-20 px-6 max-w-7xl mx-auto min-h-[90vh] flex flex-col lg:flex-row items-center gap-16">
        ...
        <div className="w-full lg:w-3/5 relative">
          <motion.div 
            ...
            className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[80%] h-[80%] rounded-full opacity-30 blur-[80px] -z-10"
          ></motion.div>
          
          {/* Nền ảnh xe chuyển sang tone hồng nhạt */}
          <div className="w-full aspect-[4/3] lg:aspect-square bg-pink-50/50 rounded-[3rem] p-10 flex items-center justify-center border border-pink-100 relative overflow-hidden">
            <AnimatePresence mode="wait">
              <motion.img 
                ...
                src={activeColor.img} 
                alt={`Xe điện Sweetea màu ${activeColor.name}`} 
                className="w-full h-full object-contain drop-shadow-2xl z-10 cursor-grab active:cursor-grabbing" // Con trỏ kéo thả
                drag="x" // Vuốt ngang
            ...
            <div className="absolute bottom-10 left-0 right-0 text-center pointer-events-none">
              <span className="text-[90px] lg:text-[140px] font-semibold text-pink-500/[0.04] tracking-tighter uppercase leading-none select-none">
                SWEETEA
              </span>
            </div>
          </div>
          ...
          <div className="mt-8 lg:hidden flex flex-col items-center justify-center">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-pink-900 mb-4">
              Màu sắc: <span className="text-pink-500">{activeColor.name}</span>
            </p>
            <div className="flex flex-wrap justify-center gap-4">
              {sweeteaData.colors.map((color) => (
                <button 
                  key={`mobile-${color.name}`}
                  onClick={() => setActiveColor(color)}
                  className={`relative w-12 h-12 rounded-full flex items-center justify-center transition-all shadow-sm ${activeColor.name === color.name ? "ring-2 ring-offset-4 ring-pink-400 scale-110" : "ring-1 ring-pink-200 hover:ring-pink-300 hover:scale-110"}`}
                  style={{ backgroundColor: color.hex }}
                  title={color.name}
                >
                  {activeColor.name === color.name && <Check size={20} className="text-black/70" />}
                </button>
              ))}
            </div>
        ...
        <div className="w-full lg:w-2/5 flex flex-col justify-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <p className="text-sm font-semibold tracking-[0.2em] uppercase mb-3 text-pink-500">Powelldd E-Scooter</p>
            <h1 className="text-5xl md:text-7xl font-semibold mb-4 tracking-tighter leading-none text-pink-600">{sweeteaData.name}</h1>
            <p className="text-xl md:text-2xl font-light text-pink-400 mb-8 italic">{sweeteaData.tagline}</p>
            
            <p className="text-4xl font-semibold tracking-tight text-pink-900 mb-10">{sweeteaData.price}</p>
            
            <p className="text-pink-950/70 text-sm leading-relaxed mb-10 font-light text-justify">
              {sweeteaData.description}
            </p>

            {/* BẢNG CHỌN MÀU - CHỈ HIỂN THỊ TRÊN DESKTOP */}
            <div className="mb-12 hidden lg:block">
              <div className="flex items-center justify-between mb-4">
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-pink-900">Màu sắc</p>
                <p className="text-xs font-medium text-pink-500">{activeColor.name}</p>
              </div>
              <div className="flex flex-wrap gap-4">
                {sweeteaData.colors.map((color) => (
                  <button 
                    ...
                    title={color.name}
                  >
                    {/* Dấu tick auto màu tối do các tone màu của Sweetea đều là màu sáng pastel */}
                    {activeColor.name === color.name && <Check size={20} className="text-black/70" />}
                  </button>
                ))}
              </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <a href="tel:0917747777" className="flex-1 bg-pink-500 text-white px-8 py-5 rounded-full font-medium uppercase tracking-[0.2em] text-sm text-center hover:bg-pink-600 hover:scale-105 transition-all shadow-lg shadow-pink-500/30">
                Gọi Hotline
              </a>
              <a href="https://zalo.me/0917747777" target="_blank" rel="noreferrer" className="flex-1 bg-white border border-pink-500 text-pink-600 px-8 py-5 rounded-full font-medium uppercase tracking-[0.2em] text-sm text-center hover:bg-pink-50 transition-colors">
                Nhắn Zalo ngay
              </a>
            </div>
          </motion.div>
        ...
        <div className="max-w-6xl mx-auto px-6 flex flex-col gap-12">
          {featureImages.map((src, index) => (
            <motion.div
              ...
              className="w-full rounded-[2rem] overflow-hidden shadow-sm hover:shadow-2xl transition-shadow duration-500 border border-pink-100"
            >
              <img 
                src={src} 
                alt={`Tính năng Sweetea ${index + 1}`} 
                className="w-full h-auto object-cover hover:scale-105 transition-transform duration-700" 
                onError={(e) => { e.currentTarget.style.display = 'none'; }} // Ẩn ảnh nếu sai đường dẫn
              />
            </motion.div>
          ))}
        </div>
      ...
      {/* THÔNG SỐ KỸ THUẬT (Nền hồng nhạt pastel) */}
      <section className="py-24 bg-pink-50/50 border-t border-pink-100">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-semibold uppercase tracking-widest mb-4 text-pink-900">Thông số <span className="font-light italic text-transparent bg-clip-text bg-gradient-to-r from-pink-500 to-pink-300">Kỹ thuật</span></h2>
            <div className="w-12 h-px bg-pink-300 mx-auto"></div>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {sweeteaData.specs.map((spec, idx) => (
              <motion.div 
                key={idx}
                ...
                viewport={{ once: true }}
                className="bg-white p-8 rounded-[2rem] border border-pink-100 flex items-start gap-5 hover:shadow-xl hover:shadow-pink-200/50 transition-all group"
              >
                <div className="w-14 h-14 rounded-2xl bg-pink-50 flex items-center justify-center text-pink-500 group-hover:bg-pink-500 group-hover:text-white transition-colors shrink-0">
                  {spec.icon}
                </div>
                <div>
                  <p className="text-[10px] font-semibold text-pink-400 tracking-widest uppercase mb-1">{spec.label}</p>
                  <p className="text-lg font-medium text-pink-950">{spec.value}</p>
                </div>
              </motion.div>
            ))}
          </div>
