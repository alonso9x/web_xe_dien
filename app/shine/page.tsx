      ...
      <header className="fixed w-full top-0 bg-white/50 backdrop-blur-xl z-50 border-b border-orange-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex flex-col items-end">
            <h1 className="text-xl font-bold tracking-widest uppercase text-transparent bg-clip-text bg-gradient-to-r from-orange-600 to-rose-600 leading-none">Minh Anh</h1>
            <span className="text-[9px] font-bold tracking-widest text-orange-500 uppercase mt-1">E-Scooter</span>
          </div>
        </div>
      </header>
      ...
      <section className="pt-28 pb-20 px-6 max-w-7xl mx-auto min-h-[90vh] flex flex-col lg:flex-row items-center gap-16">
        ...
        <div className="w-full lg:w-3/5 relative">
          ...
          <div className="w-full aspect-[4/3] lg:aspect-square bg-gradient-to-br from-orange-200 to-rose-200 rounded-[3rem] p-10 flex items-center justify-center border-4 border-white shadow-2xl shadow-rose-200 relative overflow-hidden">
            ...
            <div className="absolute bottom-10 left-0 right-0 text-center pointer-events-none">
              <span className="text-[100px] lg:text-[160px] font-bold text-rose-500/[0.08] tracking-tighter uppercase leading-none select-none">
                SHINE
              </span>
            </div>
          </div>
          ...
          <div className="mt-8 lg:hidden flex flex-col items-center justify-center">
            ...
            <div className="flex flex-wrap justify-center gap-4">
              {shineData.colors.map((color) => (
                <button 
                  key={`mobile-${color.name}`}
                  onClick={() => setActiveColor(color)}
                  className={`relative w-12 h-12 rounded-full flex items-center justify-center transition-all shadow-md ${activeColor.name === color.name ? "ring-4 ring-offset-4 ring-offset-orange-50 ring-rose-500 scale-110" : "ring-2 ring-white hover:ring-rose-300 hover:scale-110"}`}
            ...
            <p className="text-[10px] text-rose-500 mt-4 italic font-medium">← Vuốt ảnh để xem màu khác →</p>
          </div>
        </div>
        ...
        <div className="w-full lg:w-2/5 flex flex-col justify-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <h1 className="text-5xl md:text-7xl font-bold mb-4 tracking-tighter leading-none text-transparent bg-clip-text bg-gradient-to-r from-orange-500 to-rose-600">{shineData.name}</h1>
            <p className="text-xl md:text-2xl font-medium text-rose-500 mb-8 italic">{shineData.tagline}</p>
            
            <p className="text-4xl font-bold tracking-tight text-rose-900 mb-10">{shineData.price}</p>
            
            <p className="text-rose-950 text-sm leading-relaxed mb-10 font-medium text-justify">
            ...
            <div className="mb-12 hidden lg:block">
              <div className="flex items-center justify-between mb-4">
                ...
                <p className="text-xs font-bold text-rose-600">{activeColor.name}</p>
              </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <a href="tel:0917747777" className="flex-1 bg-gradient-to-r from-orange-500 to-rose-500 text-white px-8 py-5 rounded-full font-bold uppercase tracking-[0.2em] text-sm text-center hover:scale-105 transition-transform shadow-xl shadow-rose-500/40">
                Gọi Hotline
              </a>
              <a href="https://zalo.me/0917747777" target="_blank" rel="noreferrer" className="flex-1 bg-white border-2 border-rose-300 text-rose-600 px-8 py-5 rounded-full font-bold uppercase tracking-[0.2em] text-sm text-center hover:bg-rose-50 hover:border-rose-500 transition-colors shadow-lg">
                Nhắn Zalo ngay
              </a>
            </div>
          </motion.div>
        ...
        <div className="max-w-6xl mx-auto px-6 flex flex-col gap-12">
          {featureImages.map((src, index) => (
            <motion.div
              ...
              className="w-full rounded-[2rem] overflow-hidden shadow-xl hover:shadow-2xl hover:shadow-rose-500/20 transition-all duration-500 border-4 border-white"
            >
              <img 
                src={src} 
                alt={`Tính năng Shine ${index + 1}`} 
      ...
      <section className="py-24 bg-rose-100 border-t border-rose-200">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold uppercase tracking-widest mb-4 text-rose-950">Thông số <span className="italic text-transparent bg-clip-text bg-gradient-to-r from-orange-500 to-rose-600">Kỹ thuật</span></h2>
            <div className="w-16 h-1 bg-orange-400 mx-auto rounded-full"></div>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {shineData.specs.map((spec, idx) => (
              <motion.div 
                key={idx}
              ...
              >
                ...
                <div>
                  <p className="text-[10px] font-bold text-orange-600 tracking-widest uppercase mb-1">{spec.label}</p>
                  <p className="text-lg font-bold text-rose-950">{spec.value}</p>
                </div>
              </motion.div>
            ))}
          </div>
