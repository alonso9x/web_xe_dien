...
const eikoData = {
  ...
  colors: [
    { name: "Đỏ Fire", hex: "#EF4444", img: "/images/powelldd/eiko/do-fire.png" },
    { name: "Trắng Ice", hex: "#F8FAFC", img: "/images/powelldd/eiko/trang-ice.png" },
    { name: "Xanh Mint", hex: "#6EE7B7", img: "/images/powelldd/eiko/xanh-mint.png" },
    { name: "Xám Silver", hex: "#9CA3AF", img: "/images/powelldd/eiko/xam-silver.png" },
    { name: "Đen Neon", hex: "#111827", img: "/images/powelldd/eiko/den-neon.png" }
  ],
  specs: [
    { icon: <Zap size={20} strokeWidth={1.5}/>, label: "Động cơ", value: "600W - Khởi động êm ái" },
    { icon: <BatteryCharging size={20} strokeWidth={1.5}/>, label: "Pin/Ắc quy", value: "Graphene 48V22Ah" },
    { icon: <Ruler size={20} strokeWidth={1.5}/>, label: "Quãng đường", value: "60 km / 1 lần sạc" },
    { icon: <Gauge size={20} strokeWidth={1.5}/>, label: "Vận tốc tối đa", value: "32 km/h" },
    { icon: <Weight size={20} strokeWidth={1.5}/>, label: "Tải trọng", value: "Baga sau tải 40kg" },
    { icon: <ShieldCheck size={20} strokeWidth={1.5}/>, label: "Tiện ích", value: "iMA AI & Smartkey Pro" }
  ]
};

const featureImages = [
  "/images/vehicles/eiko/anh(1).png",
  "/images/vehicles/eiko/anh(2).png",
  "/images/vehicles/eiko/anh(3).jpg",
  "/images/vehicles/eiko/anh(4).jpg",
  "/images/vehicles/eiko/anh(5).jpg"
];

export default function EikoDetail() {
  const [activeColor, setActiveColor] = useState(eikoData.colors[0]);

  const handleDragEnd = (event: any, info: any) => {
    const swipeThreshold = 50;
    const currentIndex = eikoData.colors.findIndex(c => c.name === activeColor.name);

    if (info.offset.x < -swipeThreshold) {
      const nextIndex = (currentIndex + 1) % eikoData.colors.length;
      setActiveColor(eikoData.colors[nextIndex]);
    } else if (info.offset.x > swipeThreshold) {
      const prevIndex = (currentIndex - 1 + eikoData.colors.length) % eikoData.colors.length;
      setActiveColor(eikoData.colors[prevIndex]);
    }
  };

  return (
    <main className={`min-h-screen bg-gradient-to-br from-red-50 via-orange-50/50 to-slate-50/40 text-slate-800 ${elegantFont.className} font-light selection:bg-orange-200 selection:text-red-600`}>
      
      <header className="fixed w-full top-0 bg-red-50/70 backdrop-blur-xl z-50 border-b border-red-100">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          <Link href="/" className="flex items-center gap-2 text-red-500 hover:text-slate-700 transition-colors group">
            <ArrowLeft size={20} className="group-hover:-translate-x-1 transition-transform" />
            <span className="text-sm font-semibold tracking-widest uppercase mt-0.5">Trở về</span>
          </Link>
          <div className="flex flex-col items-end">
            <h1 className="text-xl font-bold tracking-widest uppercase text-transparent bg-clip-text bg-gradient-to-r from-red-600 to-orange-500 leading-none">Minh Anh</h1>
            <span className="text-[9px] font-bold tracking-widest text-red-600 uppercase mt-1">E-Scooter</span>
          </div>
        </div>
      </header>
      ...
      <section className="pt-28 pb-20 px-6 max-w-7xl mx-auto min-h-[90vh] flex flex-col lg:flex-row items-center gap-16">
        <div className="w-full lg:w-3/5 relative">
          ...
          <div className="w-full aspect-[4/3] lg:aspect-square bg-gradient-to-br from-red-100/60 to-orange-100/40 rounded-[3rem] p-10 flex items-center justify-center border-4 border-red-50 shadow-xl shadow-red-200/40 relative overflow-hidden">
            <AnimatePresence mode="wait">
              <motion.img 
                ...
                src={activeColor.img} 
                alt={`Xe điện Eiko màu ${activeColor.name}`} 
            ...
            <div className="absolute bottom-10 left-0 right-0 text-center pointer-events-none">
              <span className="text-[100px] lg:text-[180px] font-bold text-red-400/[0.06] tracking-tighter uppercase leading-none select-none">
                EIKO
              </span>
            </div>
          </div>
          ...
          <div className="mt-8 lg:hidden flex flex-col items-center justify-center">
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-red-700 mb-4">
              Màu sắc: <span className="text-red-500">{activeColor.name}</span>
            </p>
            <div className="flex flex-wrap justify-center gap-4">
              {eikoData.colors.map((color) => (
                <button 
                  key={`mobile-${color.name}`}
                  onClick={() => setActiveColor(color)}
                  className={`relative w-12 h-12 rounded-full flex items-center justify-center transition-all shadow-md ${activeColor.name === color.name ? "ring-4 ring-offset-4 ring-offset-red-50 ring-red-400 scale-110" : "ring-2 ring-red-100 hover:ring-red-300 hover:scale-110"}`}
                  style={{ backgroundColor: color.hex }}
                  title={color.name}
                >
                  {activeColor.name === color.name && <Check size={20} className={color.hex === "#F8FAFC" ? "text-slate-800 drop-shadow-sm" : "text-white drop-shadow-sm"} />}
                </button>
              ))}
            </div>
            <p className="text-[10px] text-red-400 mt-4 italic font-medium">← Vuốt ảnh để xem màu khác →</p>
          </div>
        </div>

        <div className="w-full lg:w-2/5 flex flex-col justify-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <p className="text-sm font-bold tracking-[0.2em] uppercase mb-3 text-red-500">Powelldd E-Scooter</p>
            <h1 className="text-5xl md:text-7xl font-bold mb-4 tracking-tighter leading-none text-slate-800">{eikoData.name}</h1>
            <p className="text-xl md:text-2xl font-medium text-red-400 mb-8 italic">{eikoData.tagline}</p>
            
            <p className="text-4xl font-bold tracking-tight text-slate-800 mb-10">{eikoData.price}</p>
            
            <p className="text-slate-600 text-sm leading-relaxed mb-10 font-medium text-justify">
              {eikoData.description}
            </p>

            <div className="mb-12 hidden lg:block">
              <div className="flex items-center justify-between mb-4">
                <p className="text-xs font-bold uppercase tracking-[0.2em] text-red-700">Màu sắc</p>
                <p className="text-xs font-bold text-red-500">{activeColor.name}</p>
              </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <a href="tel:0917747777" className="flex-1 bg-gradient-to-r from-red-500 to-orange-500 text-white px-8 py-5 rounded-full font-bold uppercase tracking-[0.2em] text-sm text-center hover:scale-105 transition-transform shadow-lg shadow-red-500/30">
                Gọi Hotline
              </a>
              <a href="https://zalo.me/0917747777" target="_blank" rel="noreferrer" className="flex-1 bg-red-50/50 border-2 border-red-300 text-red-600 px-8 py-5 rounded-full font-bold uppercase tracking-[0.2em] text-sm text-center hover:bg-red-100 hover:border-red-400 transition-colors shadow-sm">
                Nhắn Zalo ngay
              </a>
            </div>
          </motion.div>
      ...
      <section className="py-20 bg-gradient-to-b from-transparent to-red-100/40">
        <div className="max-w-6xl mx-auto px-6 flex flex-col gap-12">
          {featureImages.map((src, index) => (
            <motion.div
              ...
              className="w-full rounded-[2rem] overflow-hidden shadow-md hover:shadow-2xl hover:shadow-red-500/10 transition-all duration-500 border-4 border-red-50"
            >
              <img 
                src={src} 
                alt={`Tính năng Eiko ${index + 1}`} 
      ...
      <section className="py-24 bg-gradient-to-b from-red-50/80 to-slate-50/50 border-t border-red-100">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold uppercase tracking-widest mb-4 text-slate-800">Thông số <span className="italic text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-orange-500">Kỹ thuật</span></h2>
            <div className="w-16 h-1 bg-red-300 mx-auto rounded-full"></div>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {eikoData.specs.map((spec, idx) => (
              <motion.div 
                key={idx}
                ...
                viewport={{ once: true }}
                className="bg-red-50/60 backdrop-blur-sm p-8 rounded-[2rem] border-2 border-red-100 flex items-start gap-5 hover:shadow-xl hover:shadow-red-200/30 hover:border-red-300 transition-all group"
              >
                <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-red-500 to-orange-500 flex items-center justify-center text-white shadow-sm group-hover:scale-110 group-hover:from-red-400 group-hover:to-orange-400 transition-all shrink-0">
                  {spec.icon}
                </div>
                <div>
                  <p className="text-[10px] font-bold text-red-600 tracking-widest uppercase mb-1">{spec.label}</p>
