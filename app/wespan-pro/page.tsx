...
const wespanData = {
  ...
  price: "20.500.000 VNĐ",
  signatureHex: "#D97706",
  description: "Mang đậm hơi thở cổ điển Châu Âu, Powelldd Wespan Pro là biểu tượng của sự thanh lịch và sang trọng. Sở hữu đường cong vuốt mềm mại kết hợp cùng công nghệ động cơ mạnh mẽ, Wespan Pro không chỉ là một phương tiện di chuyển, mà còn là một tuyệt tác nghệ thuật tôn lên phong cách sống tinh tế của người sở hữu.",
  colors: [
    { name: "Đen vũ trụ", hex: "#1A1A1A", img: "/images/powelldd/wespan-pro/den-vu-tru.png" },
    { name: "Trắng băng sương", hex: "#F4F4F4", img: "/images/powelldd/wespan-pro/trang-bang-suong.png" },
    { name: "Xám bạc khí", hex: "#C0C0C0", img: "/images/powelldd/wespan-pro/xam-bac-khi.png" },
    { name: "Xanh titan", hex: "#4A5A75", img: "/images/powelldd/wespan-pro/xanh-titan.png" }
  ],
  specs: [
    ...
    { icon: <Gauge size={20} strokeWidth={1.5}/>, label: "Vận tốc tối đa", value: "47 km/h" },
      ...
      <header className="fixed w-full top-0 bg-[#F4F4F5]/80 backdrop-blur-xl z-50 border-b border-zinc-300 shadow-sm shadow-zinc-400/30">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex flex-col items-end">
            <h1 className="text-xl font-bold tracking-widest uppercase text-zinc-900 leading-none">Minh Anh</h1>
            <span className="text-[9px] font-bold tracking-widest text-zinc-500 uppercase mt-1">E-Scooter</span>
          </div>
        </div>
      </header>
      ...
      <section className="pt-28 pb-20 px-6 max-w-7xl mx-auto min-h-[90vh] flex flex-col lg:flex-row items-center gap-16">
        ...
        <div className="w-full lg:w-3/5 relative">
          <motion.div 
            ...
            className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[80%] h-[80%] rounded-full opacity-30 blur-[90px] -z-10 mix-blend-multiply"
          ></motion.div>
          
          <div className="w-full aspect-[4/3] lg:aspect-square bg-gradient-to-br from-zinc-50 via-zinc-100 to-zinc-300 rounded-[3rem] p-10 flex items-center justify-center border-2 border-white shadow-[0_20px_50px_-12px_rgba(82,82,91,0.3)] relative overflow-hidden">
            ...
            <div className="absolute bottom-10 left-0 right-0 text-center pointer-events-none">
              <span className="text-[100px] lg:text-[150px] font-bold text-zinc-900/[0.04] tracking-tighter uppercase leading-none select-none mix-blend-overlay">
                WESPAN
              </span>
            </div>
          </div>
          ...
          <div className="mt-8 lg:hidden flex flex-col items-center justify-center">
            ...
            <div className="flex flex-wrap justify-center gap-4">
              {wespanData.colors.map((color) => (
                <button 
                  key={`mobile-${color.name}`}
                  onClick={() => setActiveColor(color)}
                  className={`relative w-12 h-12 rounded-full flex items-center justify-center transition-all shadow-md ${activeColor.name === color.name ? "ring-2 ring-offset-4 ring-offset-[#E4E4E7] ring-zinc-800 scale-110" : "ring-1 ring-zinc-400 hover:ring-zinc-500 hover:scale-110"}`}
                  style={{ backgroundColor: color.hex }}
                  title={color.name}
                >
                  {activeColor.name === color.name && (color.hex === "#F4F4F4" || color.hex === "#C0C0C0") && <Check size={20} className="text-zinc-900 drop-shadow-sm" />}
                  {activeColor.name === color.name && color.hex !== "#F4F4F4" && color.hex !== "#C0C0C0" && <Check size={20} className="text-white drop-shadow-sm" />}
                </button>
              ))}
            </div>
            <p className="text-[10px] text-zinc-500 mt-4 italic font-medium">← Vuốt ảnh để xem màu khác →</p>
          </div>
        </div>
        ...
        <div className="w-full lg:w-2/5 flex flex-col justify-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <h1 className="text-5xl md:text-7xl font-bold mb-4 tracking-tighter leading-none text-transparent bg-clip-text bg-gradient-to-r from-zinc-700 to-zinc-950">{wespanData.name}</h1>
            <p className="text-xl md:text-2xl font-medium text-zinc-500 mb-8 italic">{wespanData.tagline}</p>
            
            <p className="text-4xl font-bold tracking-tight text-zinc-900 mb-10">{wespanData.price}</p>
            
            <p className="text-zinc-600 text-sm leading-relaxed mb-10 font-medium text-justify">
            ...
            <div className="mb-12 hidden lg:block">
              <div className="flex items-center justify-between mb-4">
                ...
                <p className="text-xs font-bold text-zinc-500">{activeColor.name}</p>
              </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <a href="tel:0917747777" className="flex-1 bg-gradient-to-r from-zinc-700 to-zinc-900 text-zinc-100 px-8 py-5 rounded-full font-bold uppercase tracking-[0.2em] text-sm text-center hover:scale-105 transition-transform shadow-xl shadow-zinc-900/30 border border-zinc-600">
                Gọi Hotline
              </a>
              <a href="https://zalo.me/0917747777" target="_blank" rel="noreferrer" className="flex-1 bg-zinc-100/50 border-2 border-zinc-400 text-zinc-800 px-8 py-5 rounded-full font-bold uppercase tracking-[0.2em] text-sm text-center hover:bg-zinc-200 hover:border-zinc-500 transition-colors shadow-sm">
                Nhắn Zalo ngay
              </a>
            </div>
          </motion.div>
        ...
        <div className="max-w-6xl mx-auto px-6 flex flex-col gap-12">
          {featureImages.map((src, index) => (
            <motion.div
              ...
              className="w-full rounded-[2rem] overflow-hidden shadow-lg hover:shadow-[0_25px_50px_-12px_rgba(82,82,91,0.25)] transition-all duration-500 border-2 border-zinc-50/50"
            >
              <img 
                src={src} 
                alt={`Tính năng Wespan Pro ${index + 1}`} 
      ...
      <section className="py-24 bg-gradient-to-b from-zinc-200/80 to-zinc-300/50 border-t border-zinc-300">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold uppercase tracking-widest mb-4 text-zinc-900">Thông số <span className="italic text-transparent bg-clip-text bg-gradient-to-r from-zinc-600 to-zinc-400">Kỹ thuật</span></h2>
            <div className="w-16 h-1 bg-zinc-500 mx-auto rounded-full"></div>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {wespanData.specs.map((spec, idx) => (
              <motion.div 
                key={idx}
                ...
                viewport={{ once: true }}
                className="bg-gradient-to-br from-zinc-50 to-zinc-100 p-8 rounded-[2rem] border border-zinc-300 flex items-start gap-5 hover:shadow-2xl hover:shadow-zinc-500/30 hover:border-zinc-400 transition-all group"
              >
                <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-zinc-200 to-zinc-300 flex items-center justify-center text-zinc-700 group-hover:from-zinc-700 group-hover:to-zinc-900 group-hover:text-zinc-100 transition-all shrink-0 shadow-inner border border-zinc-100">
                  {spec.icon}
                </div>
                <div>
                  <p className="text-[10px] font-bold text-zinc-500 tracking-widest uppercase mb-1">{spec.label}</p>
                  <p className="text-lg font-bold text-zinc-800">{spec.value}</p>
                </div>
              </motion.div>
            ))}
          </div>
