...
const dinaData = {
  name: "Dina",
  tagline: "Thông minh & Thanh lịch",
  price: "14.490.000 VNĐ",
  signatureHex: "#F472B6",
  description: "Powelldd Dina gây ấn tượng mạnh mẽ với hệ điều hành thông minh iMA AI và khả năng kết nối App Powelldd Go hiện đại. Sở hữu vóc dáng thon gọn nhưng đầy nội lực với khối động cơ 800W, kết hợp cùng hàng loạt công nghệ an toàn ưu việt như TCS chống trượt, hỗ trợ lên xuống dốc. Dina chính là mảnh ghép hoàn hảo cho những tâm hồn tự do và đầy cá tính.",
  colors: [
    { name: "Hồng Blush", hex: "#F472B6", img: "/images/powelldd/dina/hong-blush.png" },
    { name: "Trắng Frost", hex: "#F4F4F4", img: "/images/powelldd/dina/trang-frost.png" }, 
    { name: "Bạc Mist", hex: "#D1D5DB", img: "/images/powelldd/dina/bac-mist.png" },
    { name: "Xám Urban", hex: "#9CA3AF", img: "/images/powelldd/dina/xam-urban.png" },
    { name: "Đen Phantom", hex: "#1A1A1A", img: "/images/powelldd/dina/den-phantom.png" }
  ],
  specs: [
    { icon: <Zap size={20} strokeWidth={1.5}/>, label: "Động cơ", value: "800W (Cực đại 1592W)" },
    { icon: <BatteryCharging size={20} strokeWidth={1.5}/>, label: "Pin/Ắc quy", value: "60V-22Ah" },
    { icon: <Ruler size={20} strokeWidth={1.5}/>, label: "Quãng đường", value: "80 km / 1 lần sạc" },
    { icon: <Gauge size={20} strokeWidth={1.5}/>, label: "Vận tốc tối đa", value: "40 km/h" },
    { icon: <Weight size={20} strokeWidth={1.5}/>, label: "Trọng lượng", value: "51.65 kg (Nhẹ nhàng)" },
    { icon: <ShieldCheck size={20} strokeWidth={1.5}/>, label: "An toàn", value: "TCS chống trượt & Phanh đĩa" }
  ]
};

const featureImages = [
  "/images/vehicles/dina/1.png",
  "/images/vehicles/dina/2.png",
  "/images/vehicles/dina/3.png",
  "/images/vehicles/dina/4.png",
  "/images/vehicles/dina/5.png",
  "/images/vehicles/dina/6.png"
];

const galleryImages = Array.from({ length: 6 }, (_, i) => `/images/vehicles/dina/gallery/anh(${i + 1}).jpg`);

export default function DinaDetail() {
  const [activeColor, setActiveColor] = useState(dinaData.colors[0]);

  const handleDragEnd = (event: any, info: any) => {
    const swipeThreshold = 50; 
    const currentIndex = dinaData.colors.findIndex(c => c.name === activeColor.name);

    if (info.offset.x < -swipeThreshold) {
      const nextIndex = (currentIndex + 1) % dinaData.colors.length;
      setActiveColor(dinaData.colors[nextIndex]);
    } else if (info.offset.x > swipeThreshold) {
      const prevIndex = (currentIndex - 1 + dinaData.colors.length) % dinaData.colors.length;
      setActiveColor(dinaData.colors[prevIndex]);
    }
  };

  return (
    <main className={`min-h-screen bg-[#FDFBF7] text-pink-950 ${elegantFont.className} font-light selection:bg-pink-200 selection:text-pink-900`}>
      <header className="fixed w-full top-0 bg-[#FDFBF7]/80 backdrop-blur-xl z-50 border-b border-pink-200/60 shadow-sm shadow-pink-900/5">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          <Link href="/" className="flex items-center gap-2 text-pink-700 hover:text-pink-900 transition-colors group">
            <ArrowLeft size={20} className="group-hover:-translate-x-1 transition-transform" />
            <span className="text-sm font-semibold tracking-widest uppercase mt-0.5">Trở về</span>
          </Link>
          <div className="flex flex-col items-end">
            <h1 className="text-xl font-bold tracking-widest uppercase text-pink-900 leading-none">Minh Anh</h1>
            <span className="text-[9px] font-bold tracking-widest text-pink-600 uppercase mt-1">E-Scooter</span>
          </div>
        </div>
      </header>
      ...
      <section className="pt-28 pb-20 px-6 max-w-7xl mx-auto min-h-[90vh] flex flex-col lg:flex-row items-center gap-16">
        <div className="w-full lg:w-3/5 relative">
          ...
          <div className="w-full aspect-[4/3] lg:aspect-square bg-gradient-to-br from-[#FFFDF9] to-[#FDF2F8] rounded-[3rem] p-10 flex items-center justify-center border-2 border-pink-100 shadow-[0_20px_50px_-12px_rgba(244,114,182,0.15)] relative overflow-hidden">
            <AnimatePresence mode="wait">
              <motion.img 
                ...
                src={activeColor.img} 
                alt={`Xe điện Dina màu ${activeColor.name}`} 
            ...
            <div className="absolute bottom-10 left-0 right-0 text-center pointer-events-none">
              <span className="text-[120px] lg:text-[180px] font-bold text-pink-900/[0.04] tracking-tighter uppercase leading-none select-none">
                DINA
              </span>
            </div>
          </div>
          ...
          <div className="mt-8 lg:hidden flex flex-col items-center justify-center">
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-pink-900 mb-4">
              Màu sắc: <span className="text-pink-600">{activeColor.name}</span>
            </p>
            <div className="flex flex-wrap justify-center gap-4">
              {dinaData.colors.map((color) => (
                <button 
                  key={`mobile-${color.name}`}
                  onClick={() => setActiveColor(color)}
                  className={`relative w-12 h-12 rounded-full flex items-center justify-center transition-all shadow-md ${activeColor.name === color.name ? "ring-2 ring-offset-4 ring-offset-[#FDFBF7] ring-pink-500 scale-110" : "ring-1 ring-pink-200 hover:ring-pink-400 hover:scale-110"}`}
                  style={{ backgroundColor: color.hex }}
                  title={color.name}
                >
                  {activeColor.name === color.name && (color.hex === "#F4F4F4" || color.hex === "#D1D5DB" || color.hex === "#9CA3AF") && <Check size={20} className="text-neutral-900" />}
                  {activeColor.name === color.name && color.hex !== "#F4F4F4" && color.hex !== "#D1D5DB" && color.hex !== "#9CA3AF" && <Check size={20} className="text-[#FDFBF7]" />}
                </button>
              ))}
            </div>
            <p className="text-[10px] text-pink-700/60 mt-4 italic font-medium">← Vuốt ảnh để xem màu khác →</p>
          </div>
        </div>

        <div className="w-full lg:w-2/5 flex flex-col justify-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <p className="text-sm font-bold tracking-[0.2em] uppercase mb-3" style={{ color: dinaData.signatureHex }}>Powelldd E-Scooter</p>
            <h1 className="text-5xl md:text-7xl font-bold mb-4 tracking-tighter leading-none text-pink-950">{dinaData.name}</h1>
            <p className="text-xl md:text-2xl font-medium text-pink-700/70 mb-8 italic">{dinaData.tagline}</p>
            
            <p className="text-4xl font-bold tracking-tight text-pink-900 mb-10">{dinaData.price}</p>
            
            <p className="text-pink-900/80 text-sm leading-relaxed mb-10 font-medium text-justify">
              {dinaData.description}
            </p>

            <div className="mb-12 hidden lg:block">
              <div className="flex items-center justify-between mb-4">
                <p className="text-xs font-bold uppercase tracking-[0.2em] text-pink-900">Màu sắc</p>
                <p className="text-xs font-bold text-pink-600">{activeColor.name}</p>
              </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <a href="tel:0917747777" className="flex-1 bg-gradient-to-r from-pink-600 to-pink-500 text-[#FDFBF7] px-8 py-5 rounded-full font-bold uppercase tracking-[0.2em] text-sm text-center hover:scale-105 transition-transform shadow-xl shadow-pink-900/20 border border-pink-700">
                Gọi Hotline
              </a>
              <a href="https://zalo.me/0917747777" target="_blank" rel="noreferrer" className="flex-1 bg-transparent border-2 border-pink-500/50 text-pink-800 px-8 py-5 rounded-full font-bold uppercase tracking-[0.2em] text-sm text-center hover:bg-pink-100/50 hover:border-pink-600 transition-colors shadow-sm">
                Nhắn Zalo ngay
              </a>
            </div>
          </motion.div>
      ...
      <section className="py-20 bg-gradient-to-b from-[#FDFBF7] to-[#FDF2F8]/50">
        <div className="max-w-6xl mx-auto px-6 flex flex-col gap-12">
          {featureImages.map((src, index) => (
            <motion.div
              ...
              className="w-full rounded-[2rem] overflow-hidden shadow-lg hover:shadow-[0_25px_50px_-12px_rgba(244,114,182,0.25)] transition-all duration-500 border-2 border-pink-100"
            >
              <img 
                src={src} 
                alt={`Tính năng Dina ${index + 1}`} 
                className="w-full h-auto object-cover hover:scale-105 transition-transform duration-1000 origin-center filter sepia-[0.05]" 
              />
            </motion.div>
          ))}
        </div>
      ...
      <section className="py-16 bg-[#FDFBF7]">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-10">
            <h2 className="text-2xl md:text-3xl font-bold uppercase tracking-widest mb-2 text-pink-950">Album <span className="italic text-transparent bg-clip-text bg-gradient-to-r from-pink-600 to-pink-400">Ảnh</span></h2>
            <div className="w-12 h-1 bg-pink-500 mx-auto rounded-full"></div>
          </div>
      ...
      <section className="py-24 bg-[#FDF2F8]/70 border-t border-pink-200/50">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold uppercase tracking-widest mb-4 text-pink-950">Thông số <span className="italic text-transparent bg-clip-text bg-gradient-to-r from-pink-600 to-pink-400">Kỹ thuật</span></h2>
            <div className="w-16 h-1 bg-pink-500 mx-auto rounded-full"></div>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {dinaData.specs.map((spec, idx) => (
              <motion.div 
                key={idx}
                ...
                viewport={{ once: true }}
                className="bg-[#FFFDF9] p-8 rounded-[2rem] border border-pink-200/80 flex items-start gap-5 hover:shadow-xl hover:shadow-pink-900/10 hover:border-pink-400 transition-all group"
              >
                <div className="w-14 h-14 rounded-2xl bg-pink-100/60 flex items-center justify-center text-pink-600 group-hover:bg-pink-600 group-hover:text-[#FFFDF9] transition-all shrink-0 shadow-sm border border-pink-200/50">
                  {spec.icon}
                </div>
                <div>
                  <p className="text-[10px] font-bold text-pink-600/70 tracking-widest uppercase mb-1">{spec.label}</p>
                  <p className="text-lg font-bold text-pink-900">{spec.value}</p>
                </div>
              </motion.div>
            ))}
          </div>
